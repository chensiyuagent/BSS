...
def precision(prediction, target):
    idx_egde_in_prediction = np.argwhere(prediction==1)
    idx_egde_in_target = np.argwhere(target==1)
...
# DataLoader
trainloader = DataLoader(trainset,batch_size=100,shuffle=True,sampler=sampler)
...
target = y_test[:, 0]
prec = precision(predict_test, target)
reca = recall(predict_test, target)
print("Accuracy:{}, Precision:{}, Recall:{}".format(acc, prec, reca))

output_matrix = np.reshape(predicted[:, 0].detach().numpy(), weight_2017.shape) 

np.savetxt('2017_prediction.csv', output_matrix, fmt='%.2e')